...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function (exports) {
    ...
    Class.prototype.render = function () {
        ...
        options.request = $.extend({
            pageName: layui.setter.response.listPageName || "page",
            limitName: layui.setter.response.listPageSizeName || "limit"
        }, options.request)

        //响应数据的自定义格式
        options.response = $.extend({
            statusName: layui.setter.response.statusName || 'code'
            , statusCode: layui.setter.response.statusCode.ok || 0
            , msgName: layui.setter.response.msgName || 'msg'
            , dataName: layui.setter.response.dataName || 'data'
            , countName: 'count'
        }, options.response);
            ...
            $.ajax({
                ...
                , headers: options.headers || {},
                 beforeSend: function (xml) {
                    var token = layui.$http.data[layui.setter.request.tokenName];
                    if (token) {
                        xml.setRequestHeader(layui.setter.request.tokenName, token);
                    }
                },
                success: function (res, status, xhr) {
                    var responseInfo = layui.setter.response;
                    var token = xhr.getResponseHeader(layui.setter.request.tokenName);
                    if (token) {
                        layui.$http.setData({
                            [layui.setter.request.tokenName]: token
                        });
                    }
                    if (!options.parseData && responseInfo.listLotalName && responseInfo.listDataName) {
                        var lotalArr = responseInfo.listLotalName.split(".");
                        var listLotal = res;
                        for (var l in lotalArr) {
                            listLotal = listLotal[lotalArr[l]];
                        } 
                        res.count = listLotal;
                        var dataArr = responseInfo.listDataName.split(".");
                        var listData = res;
                        for (var l in dataArr) {
                            listData = listData[dataArr[l]];
                        }
                        res.data = listData;
                    }
